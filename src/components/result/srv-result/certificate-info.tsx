...
export const CertificateInfo = component$<CertsProps>(({ certs, ip }) => {
  ...
  useVisibleTask$(() => {
    cert_modal.value = document.getElementById(`cert_modal_${ip}`) as HTMLDialogElement;
  });
    ...
    <div class="card bordered">
      <div class="card-body whitespace-normal">
        ...
        <div class="grid grid-cols-[auto_minmax(0,_1fr)] gap-x-4 gap-y-1">
          ...
          <div>{cert.KeyUsage && cert.KeyUsage.length > 0 ? cert.KeyUsage.join(', ') : ''}</div>

          <strong class="text-right">Extended Key Usage:</strong>
          <div>{cert.ExtKeyUsage && cert.ExtKeyUsage.length > 0 ? cert.ExtKeyUsage.join(', ') : ''}</div>
...
export const CertChainModal = component$<CertChainModalPros>(({ cert_data_id }) => {
  return (
