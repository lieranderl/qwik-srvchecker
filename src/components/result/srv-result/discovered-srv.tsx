            ...
            <>
              {Records.length > 0 &&
                Records.map((record, recordIndex) => (
                  <tr key={Srv + record.Fqdn + index.toString()} class="whitespace-nowrap">
                    {recordIndex === 0 && <td rowSpan={Records.length}>{Srv}</td>}
                    <td>
                      <div class={record.Fqdn === 'SRV record not configured' ? 'text-error' : ''}>{record.Fqdn}</div>
                    </td>
                    <td>{record.Priority}</td>
                    <td>{record.Weight}</td>
                    <td>
                      <div class={record.Ip === 'A record not configured' ? 'text-error' : ''}>{record.Ip}</div>
                    </td>
                    <td>
                      <PortCell
                        port={record.Port}
                    ...
                    <td>
                      <CertificateInfo certs={record.CertsChain} ip={record.Ip} />
                    </td>
                  </tr>
                ))}
            </>
...
export const DiscoveredSrv = component$<SrvProps>(({ srventtries, srv_filter }) => {
  return (
      ...
      <div class="card-body">
        ...
        <SrvTable
          srventtries={srventtries.filter(
            (srv) =>
              !(srv.Srv.startsWith('_cisco-uds') || srv.Srv.startsWith('_cuplogin')) &&
