...
export const TcpConnectivity = component$<ConnectivityProps>(({ connectivity, srv_filter }) => {
  const filterTcp = srv_filter
    ...
    : (p: Port) => p.Proto === 'tcp' && p.Type !== 'admin' && p.Type !== 'traversal' && p.Type !== 'turn';

  const show_tcp_table = connectivity.map((data) => data.Ports.filter(filterTcp).length > 0).includes(true);
      ...
      <div class="card-body">
        ...
        {show_tcp_table ? (
          ...
          <div class="flex flex-nowrap items-center gap-1">
            ...
            <div>Could not check port connectivity because no hosts were resolved through SRV records.</div>
          </div>
        )}
      </div>
    </div>
